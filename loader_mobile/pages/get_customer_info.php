	...
	*/
	...
	else
	{
		...
		else
		{
			...
			{
				$query1 = "SELECT name, email, postal_address ,user_token FROM view_customer_info WHERE mobile_no = '".$mobile_no."' ";
	        	loader_file_put_content('query_data','get_customer_info',$query1);
				if(loader_query($query1))
				{
					loader_file_put_content('query_data','get_customer_info',"Previous query successfully run");
					$result1 = loader_query($query1);
					if(0 == loader_num_rows($result1))
					{
						$query2 = "INSERT INTO tbl_customer_info (fld_mobile_no) VALUES('".$mobile_no."')";
						loader_file_put_content('query_data','get_customer_info',$query2);
						if(loader_query($query2))
						{
							...
							if(customer_isTokenExist($user_token))
							{
								...
								{
									loader_file_put_content('token_collisions','collided_and_removed','Customer id:'.$last_inserted_id.' Token:'.$user_token."\r\n");
									$error_message = REGISTRATION_FAILED;
								}
								else
								{
									loader_file_put_content('token_collisions','collided_not_removed','Customer id:'.$last_inserted_id.' Token:'.$user_token."\r\n");
				...
				{
					loader_file_put_content('query_data','get_customer_info',"Previous query failed");
					$error_message = SERVER_ERROR;
				}
			}
			else
	...
	loader_file_put_content('send_data','get_customer_info',$send_data);
}
