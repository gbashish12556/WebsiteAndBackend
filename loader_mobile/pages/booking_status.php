<?php
loader_file_put_content('post_data','booking_status',$_REQUEST);
if(loader_post_isset("user_token")&&loader_post_isset("crn_no"))
{
	...
	else
	{
		...
		{
			$where1 = " WHERE customer_token = '".$user_token."'";
			$query1 = "SELECT vehicletype_id,booking_datetime,crn_no,driver_token,is_booked,is_active,
	          driver_vehicle_no,driver_name,driver_mobile_no,driver_location_lat,driver_location_lng FROM view_booking_now ".$where1." ";
			loader_file_put_content('query_data','booking_status',$query1);
			if($result1 = loader_query($query1))
			{
				if(loader_num_rows($result1)>0)
				{
					...
					$userRow['likes'][0]['crn_no'] = $row['crn_no'];
