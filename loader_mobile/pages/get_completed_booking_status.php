<?php
	loader_file_put_content('post_data','get_completed_booking_status',$_REQUEST);
if(loader_post_isset('user_token')&&loader_post_isset('crn_no'))
{
    ...
    else
    {
      ...
      {
		 $query2 = "SELECT crn_no,from_point, to_point,exact_from_point, exact_to_point, booking_datetime,vehicletype_id,total_fare,driver_name,driver_mobile_no,driver_profile_pic_url, driver_rating,booking_id
		           FROM view_booking_detail WHERE (crn_no = '".$crn_no."') AND (booked_vehicle_id != 0) AND(is_completed=1)  ";
		 loader_file_put_content('query_data','get_completed_booking_status',$query2);
		 if($result2 = loader_query($query2))
		 {
			if(loader_num_rows($result2)>0)
			{
				  ...
				  $userRow['likes'][0]["driver_name"] = $row2['driver_name']."";
				  $userRow['likes'][0]["driver_mobile_no"] = $row2['driver_mobile_no']."";
					$userRow['likes'][0]["booking_id"] = $row2['booking_id']."";
					if(is_null($row2['driver_rating']))
						$userRow['likes'][0]["driver_rating"] = 0;
					else {
						$userRow['likes'][0]["driver_rating"] = $row2['driver_rating']."";
					}
					if((NULL == $row2['driver_profile_pic_url']) || ("" == $row2['driver_profile_pic_url']))
					{ 
						$userRow["likes"][0]["driver_profile_pic_url"] = "";
					}
					else
					{
						$image_path  = ('upload/'.$row2['driver_profile_pic_url']);
...
loader_file_put_content('send_data','get_completed_booking_status',$send_data);
