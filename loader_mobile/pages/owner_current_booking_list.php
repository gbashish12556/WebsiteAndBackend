...
*/
	...
	loader_file_put_content('post_data','owner_current_booking_list',$_REQUEST);
if(loader_post_isset("user_token")&&loader_post_isset("page_no"))
{
	...
	else
	{
		...
		{
			$offset = ($page_no*5);
			$where1 = "WHERE owner_token = '".$user_token."' AND customer_token != 'DefaultIsNothing' AND booking_datetime < '".$current_datetime."' AND is_completed = '0' ";
			...
			loader_file_put_content('query_data','owner_current_booking_list',$query1);
	...
	loader_file_put_content('send_data','owner_current_booking_list',$send_data);
}
