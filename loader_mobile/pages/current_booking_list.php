...
*/
	...
	loader_file_put_content('post_data','current_booking_list',$_REQUEST);
if(loader_post_isset("user_token")&&loader_post_isset("page_no"))
{
	...
	else
	{
		...
		{
			$offset  = ($page_no*5);
			$where1 = "WHERE customer_token = '".$user_token."' AND booking_datetime < '".$current_datetime."' AND is_cancelled = '0' AND is_completed = '0' ";
			...
			loader_file_put_content('query_data','current_booking_list',$query1);
			if($result1 = loader_query($query1))
			{
				if(loader_num_rows($result1)>0)
				{
					...
					while($row = loader_fetch_assoc($result1))
					{
						...
						$userRow['likes'][$i]['datetime1'] = $row['booking_datetime']."";
	...
	loader_file_put_content('send_data','current_booking_list',$send_data);
}
