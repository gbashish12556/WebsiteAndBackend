<?php
	loader_file_put_content('post_data','owner_registration',$_REQUEST);
if(loader_post_isset("mobile_no")&&loader_post_isset("OTP"))
{
	...
	else
	{
		...
		else
		{
			if(validate_phone_number($mobile_no))
			{
				...
				{
							$result="SUCCESS";
							$result = loader_send_sms("6 digit OTP for verification is ".$otp,$mobile_no,"owner_registration");
							if("SUCCESS" == $result)
							{
	...
	loader_file_put_content('send_data','owner_registration',$send_data);
}
