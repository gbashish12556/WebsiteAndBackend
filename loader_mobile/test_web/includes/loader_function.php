...
function loader_send_sms($sms_data,$receiver)
{
	//$url =  'http://sms.salert.co.in/new/api/api_http.php?';
	/*$data = 'username=ashishgupta&password=ashish123&senderid=ashishgupta&to=8276097972&
text=Hello%20world&route=Enterprise&type=text&datetime=2016-02-14%2014%3A59%3A21';*/ 
	$url =  'http://sms.salert.co.in/new/api/api_http.php?';
   ...
   // loader_file_put_content("query_data","sent_sms", $data);	
	$ch = curl_init();
	...
	//loader_file_put_content("query_data","sent_sms", $result);				
...
//push notification function for android....
function android_push($uidlist,$title,$message,$page,$id)
{

	    "SELECT p.gcm_regid, p.user_id FROM gcm_users p LEFT JOIN userinfo u ON p.user_id=u.id WHERE u.push_notification=1 AND p.user_id IN (".$uidlist.")";
		$resultAndroid = loader_query("SELECT p.gcm_regid, p.user_id FROM gcm_users p LEFT JOIN userinfo u ON p.user_id=u.id WHERE u.push_notification=1 AND p.user_id IN (".$uidlist.")");
   		$api_key = "AIzaSyBB7wlnzUu-GDDXoBBpy-bZpd21et0YJb0";
    	while($row = loader_fetch_assoc($resultAndroid)) 
    	{
    		...
    		file_put_contents("push_data/".date('d_m_Y', time())."testFile.txt", date('d/m/Y h:i:s a', time())." ".$registrationIDs."  ".print_r($message,true), FILE_APPEND);
    		$fields = array('registration_ids'  => $registrationIDs,'data' => array( "message" => $message,"title" => $title,"page" => $page,"id" => $id));
...
//push notification function for ios....

function ios_push($uidlist,$msg)
{
	$tHost = 'gateway.push.apple.com';
	$tPort = 2195;
	$tCert = 'weguess.pem';
	$tPassphrase = 'inswigo123';
	$tToken = '121c4a9a6b0b3958215a45b61394e9f848702a08349bec3a600a0547ac1d1a57';
	$tAlert = $msg;
	$tBadge = 1;
	$tSound = 'default';
	$tPayload = 'APNS Message Handled by LiveCode';		
	$result = loader_query("SELECT `devicetoken` , `deviceuid` FROM `apns_devices` WHERE `status` = 'active' AND clientid in (".$uidlist.")");
	...
	while($row = loader_fetch_assoc($result)) 
	{
		$apple_identifier = $row["deviceuid"];
		 "<br>".$deviceToken = $row['devicetoken'];
		file_put_contents("push_data/".date('d_m_Y', time())."testFile.txt", date('d/m/Y h:i:s a', time())." ".$deviceToken." ".print_r($tBody,true), FILE_APPEND);
 ...
 *********   Last Edit Date: 27/04/2016
 *********   Works Done:     Added new function definitions at the end of file
...
{
	$queryres=loader_query("SELECT email FROM view_customer_info WHERE user_token = '".$token."'");
...
{
	$queryres=loader_query("SELECT name FROM view_driver_info WHERE user_token = '".$token."'");
	if(loader_num_rows($queryres)>0)
	{
			loader_debug(__LINE__,__FILE__,__FUNCTION__);
	        return true;
