...
function loader_get_userinfo($token)
{
	$where = " WHERE fld_user_token = '".$token."'";
    $query = " SELECT fld_ai_id AS member_id, fld_user_type AS member_type, fld_name AS name,
	           fld_email AS email, fld_user_token AS token ,fld_lattitude AS lattitude, fld_longitude AS longitude, fld_is_experienced AS is_experienced, fld_push_distance AS push_distance
			   FROM view_user_info ".$where." ";
...
function loader_isValidDateTime($date)
{
	...
	$regexp = "/^([0][1-9]|[12][0-9]|3[0-1])\/([0][1-9]|1[0-2])\/(\d{4}) (0[0-9]|1[0-2])\:([0-5][0-9]) (am|pm)$/";
	loader_debug(__LINE__,__FILE__,__FUNCTION__);
...
function loader_get_address($latitude,$longitude){
